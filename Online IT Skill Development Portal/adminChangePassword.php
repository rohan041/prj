...
if(session_id()==''){
	session_start();
		...
		if(isset($_REQUEST['passupdate'])){
			...
			if($_REQUEST['aPassword'] == $_REQUEST['c_password'])
			$sql = "UPDATE admin SET admin_pass  = '$aPass' WHERE admin_email = '$aEmail'";
...
<style>
	...
	Admin Area For Forgot Password</p>
		...
		<div class="col-sm-6 col-md-4">
			<form action="" class="shadow-lg p-4" method="POST" name="forgotpass" onSubmit="return validateform()">
				<div class="form-group">
					...
					<input type="email" id="aEmail" name="aEmail" placeholder="Email" style="width: 100%; border: 1px solid #ced4da; padding: 5px; font-size: 18px; padding-left: 10px;"><br>
					<small class="form-text">we,ll  never shere your email with anyone else.</small>
				</div>
		<div class="form-group">
			<label for="rPassword">New Password</label>
			<input type="password" class="form-control" id="aPassword" placeholder="New Password" name="aPassword">
		</div>
		<div class="form-group">
...
{	
					...
					if(document.forgotpass.aEmail.value == "")
						{
						alert("Email ID should not be empty..");
						document.forgotpass.aEmail.focus();
						return false;
					}else{
							if(document.forgotpass.aPassword.value == "")
							{
								alert("Password should not be empty..");
								document.forgotpass.aPassword.focus();
								return false;
							}else{
								if(document.forgotpass.aPassword.value.length <= 8)
								{
									...
									return false;
									 ...
									 if(document.forgotpass.aPassword.value != document.forgotpass.c_password.value )
									{
